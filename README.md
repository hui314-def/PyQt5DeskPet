...
**开发时间**：从2025年8月初到今日
**开发语言**：Python 3.13
**GitHub项目地址**：https://github.com/hui314-def/PyQt5DeskPet
---
...
>💡 如需使用聊天功能，请自行下载安装[Ollama](https://ollama.com)及其大模型（教程请自搜）,
并在桌宠聊天框的设置选项中填入你安装的大模型（建议使用轻量级非推理大模型，否则回复较慢，~~如果你不介意或你的电脑配置很好当我没说~~）如果你确定电脑安装了对应模型但仍然报错，则你需要手动在cmd里输入运行模型的指令，如：ollama run deepseek-r1:1.5b来激活模型使用
...
- 添加了主题选择，可以在个性化设置中选择橙色、蓝色、绿色、粉色主题

- 支持python代码的执行，可以将python文件（如text.py）拖动到桌宠上执行相应代码，省去了一些点击操作

## ✨ 可以自由替换里面的图片和音频

- sound文件夹里面有着许多音频文件，放在同一个文件夹内的音频是随机播放的，可以根据需求自由替换，其中名为“音乐”的文件夹可以添加喜欢的歌曲。注意：1、格式为MP3，2、文件夹不要重命名，3、不建议随意删除和添加文件夹

- 当然如果你想更改images里的图片也可以，只需要注意文件夹不要重命名，文件名最好是png格式，排序方式按照数字顺序就行（如1.png, 2.png …… ）
...
桌宠正在不断开发和完善中，（~~也许我没时间搞了~~）更多功能敬请期待……
